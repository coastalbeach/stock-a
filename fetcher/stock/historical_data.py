...
class StockHistoricalData:
    ...
    def fetch_stock_history(self, stock_code, start_date, end_date, adjust=""):
        ...
        try:
            ...
            self.logger.debug(f"尝试使用东财接口获取{stock_code}的历史数据")
            df = ak.stock_zh_a_hist(symbol=stock_code, period="daily", 
                                   start_date=start_date, end_date=end_date, 
                                   adjust=adjust)
            
            if df is not None and not df.empty:
                self.logger.debug(f"成功使用东财接口获取{stock_code}的历史数据")
                return df
            
            # 如果失败，尝试使用stock_zh_a_daily接口（新浪）
            self.logger.debug(f"尝试使用新浪接口获取{stock_code}的历史数据")
            ...
            if df is not None and not df.empty:
                ...
                self.logger.debug(f"成功使用新浪接口获取{stock_code}的历史数据")
                return df
            
            # 如果前两个接口都失败，尝试使用腾讯接口
            self.logger.debug(f"尝试使用腾讯接口获取{stock_code}的历史数据")
            full_code = f"{market}{stock_code}"
            ...
            if df is not None and not df.empty:
                ...
                self.logger.debug(f"成功使用腾讯接口获取{stock_code}的历史数据")
                return df
            
            # 所有接口都失败
            self.logger.debug(f"获取{stock_code}的历史数据失败，所有接口均未返回数据")
            return None
        except Exception as e:
            # 只在debug级别记录异常信息，减少日志输出
            self.logger.debug(f"获取{stock_code}的历史数据异常: {e}")
            return None
    ...
    def save_to_database(self, df, table_name):
        ...
        try:
            ...
            #self.logger.info(f"成功保存{len(records)}条数据到{table_name}表")
            return True
        except Exception as e:
    ...
    def update_stock_data(self, stock_code):
        ...
        try:
            ...
            if not self.last_trading_day or not self.previous_trading_day:
                self.logger.warning(f"交易日信息未完全设置，无法更新{stock_code}的数据")
                return False
            ...
            if last_no_adjust_date:
                ...
                if self.last_run_in_trading_time and last_no_adjust_date == previous_trading_day_str:
                    ...
                    self.logger.debug(f"上次在交易时间运行，需要更新{stock_code}的{previous_trading_day_str}数据")
                else:
                    # 正常情况，从最后一天的下一天开始更新
            ...
            if last_hfq_date:
                ...
                if self.last_run_in_trading_time and last_hfq_date == previous_trading_day_str:
                    ...
                    self.logger.debug(f"上次在交易时间运行，需要更新{stock_code}的{previous_trading_day_str}后复权数据")
                else:
            ...
            if last_hfq_date and last_hfq_date >= last_trading_day_str and not self.last_run_in_trading_time:
                ...
                self.logger.debug(f"{stock_code}的后复权数据已是最新（{last_hfq_date}），无需更新")
            ...
            if last_no_adjust_date and last_no_adjust_date >= last_trading_day_str and not self.last_run_in_trading_time:
                ...
                self.logger.debug(f"{stock_code}的不复权数据已是最新（{last_no_adjust_date}），无需更新")
            ...
            if need_update_no_adjust and start_date_no_adjust >= previous_trading_day_str:
                self.logger.debug(f"{stock_code}的不复权数据起始日期{start_date_no_adjust}在前一交易日{previous_trading_day_str}之后，将通过最新数据更新")
            ...
            # 更新后复权数据
            if need_update_hfq:
                self.logger.debug(f"更新{stock_code}的后复权数据，从{start_date_hfq}到{last_trading_day_str}")
                ...
                if hfq_data is not None and not hfq_data.empty:
                    self.save_to_database(hfq_data, "股票历史行情_后复权")
            
            # 更新不复权数据（如果不是只使用最新数据）
            if need_update_no_adjust and not use_latest_data_only:
                self.logger.debug(f"更新{stock_code}的不复权数据，从{start_date_no_adjust}到{last_trading_day_str}")
                ...
                if no_adjust_data is not None and not no_adjust_data.empty:
                    self.save_to_database(no_adjust_data, "股票历史行情_不复权")
            
            return True
        except Exception as e:
            self.logger.error(f"更新{stock_code}的历史行情数据失败: {e}")
            return False
    ...
    def update_latest_data(self):
        ...
        try:
            ...
            return self.save_to_database(latest_data, "股票历史行情_不复权")
        except Exception as e:
